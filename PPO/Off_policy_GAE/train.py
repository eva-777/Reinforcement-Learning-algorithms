...
if is_continuous:
    action_dim = env.action_space.shape[0] # env.observation_space -> Box(low, high, shape, dtype)
else:
...
### logging ###
log_dir = './Off_policy_GAE/ppo_log'
...
### save ###
chkpt_dir = './Off_policy_GAE/ppo_save'
...
for ep_i in range(NUM_EPISODE):
    state, _ = env.reset()  # 随机初始化 -> s_0, info
    ep_return = 0
    ...
    for t in range(EPI_LEN):
        ...
        state_next, reward, done, truncation, _ = env.step(action)  # step -> state', reward, terminated, truncated, info
